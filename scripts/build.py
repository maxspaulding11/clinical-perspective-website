...
# The homepage says how big the tracker is, so the number has to come from the
# render that just happened rather than from whoever last edited the HTML. A
# stale "427 faculty" on the front page would be the exact failure the tracker
# exists to avoid, and the sweep changes it every couple of days.
_home = os.path.join(SITE, 'index.html')
with open(_home, encoding='utf-8') as _f:
    _html = _f.read()
_lede = (f'<p class="home-tracker-lede"><strong>{_t["faculty"]}</strong> faculty '
         f'confirmed accepting across <strong>{_t["posted"]}</strong> programs, '
         f'from <strong>{_t["programs"]}</strong> checked so far.</p>')
_new_html = re.sub(
    r'(<!-- home:tracker start \(generated by scripts/build\.py\) -->).*?'
    r'(<!-- home:tracker end -->)',
    lambda m: m.group(1) + '\n      ' + _lede + '\n      ' + m.group(2),
    _html, count=1, flags=re.S)
if _new_html != _html:
    with open(_home, 'w', encoding='utf-8', newline='\n') as _f:
        _f.write(_new_html)
    print(f'Homepage: tracker line updated ({_t["faculty"]} faculty, {_t["posted"]} programs).')
